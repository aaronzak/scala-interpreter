...
case class Equiv(eq: Map[LogicV, Value]) {
  ...
  def apply(v: Value): Value = v match{
    case logicV @ LogicV(id) =>{ 
      if(eq.contains(logicV)) apply(eq(logicV))
      else {logicV}
    }
    case funV @ FunV(name,args) => funV
  } // FILL ME IN
  ...
  val val1 = apply(v1)
    ...
    (val1,val2) match{
      ...
      case( logicV @ LogicV(id),mVal2: Value ) => {
          if(!mVal2.contains(logicV)) Some(Equiv(eq + (logicV->mVal2)))
          else None
        }
      case(mVal1: Value, logicV @ LogicV(id)) =>{
          if(!mVal1.contains(logicV)) Some(Equiv(eq + (logicV->mVal1)))
          else None
        }
        case (FunV(n1,args1), FunV(n2,args2)) => {
          if(n1 == n2 && args1.length == args2.length)

            unifyTerms(args1 zip args2,eq)
    ...
    def unifyTerms(pairs: Seq[(Value,Value)], eqUnify: Map[LogicV, Value]): Option[Equiv] =
    pairs.foldLeft(Option(this))((acc,(v)) => acc match{
      case Some(newEq)=> newEq.unify(v._1,v._2)
      case _ => None 
  ...
  def eval(goals: Seq[Closure], eq: Equiv): Unit =
    if (goals nonEmpty) goals.head match {
      ...
      case goalClo @ Closure(p: Predicate, env, as) ⇒
      {
       ...
       as.map((a) => a match{
        ...
        case Fact(vars,pred) => {
            ...
            //if a fact, evaluate rest of goals
            matchPredicates(Closure(pred, vars.map((x)=>x->LogicV()).toMap,as),goalClo,eq) match{
              case Some(newEq) => {
                eval(goals.tail,newEq)
              }
              case _ => Unit
            }
          }
      ...
      case Clause(vars,hd,body) => {
            val newGoals = vars.map((x)=>x->LogicV()).toMap
        
            //if clause, eval body goal
            matchPredicates(Closure(hd, newGoals,as),goalClo,eq) match{

              case Some(newEq) => eval(Closure(body, newGoals,as) +: goals.tail , newEq)
      ...
      case Closure(Conjunct(g1, g2), env, as) ⇒
         ...
         val newGoal: Seq[Closure] = Closure(g1, env, as) +: Closure(g2, env, as) +: goals.tail
        eval(newGoal,eq)
...
{
        val newGoal1: Seq[Closure] = Closure(g1, env, as) +: goals.tail
        val newGoal2: Seq[Closure] = Closure(g2, env, as) +: goals.tail
      ...
      {
        val myAs = a +: as
              val newGoal = Closure(g,env,myAs) +: goals.tail
      ...
      val newEnv = env ++ vars.map(mVar=>mVar->LogicV()).toMap
      
      val newGoal = Closure(g,newEnv,as) +: goals.tail
  ...
  def matchPredicates(aClo: Closure, gClo: Closure, eq: Equiv): Option[Equiv] ={
    ...
    (aClo.g, gClo.g) match{
      ...
      case (Predicate(n1,args1),Predicate(n2,args2)) => {   
        if(n1 != n2 || args1.length != args2.length){ 
              None
            }
            else{
              ...
              val myVal1: Seq[Value] = args1.map(termToValue(_,aClo.env))
               val myVal2: Seq[Value] = args2.map(termToValue(_, gClo.env))

               (myVal1.zip(myVal2)).foldLeft(Option(eq))((l,v) => l match{ 
                case Some(newEq) => newEq.unify(v._1,v._2)
